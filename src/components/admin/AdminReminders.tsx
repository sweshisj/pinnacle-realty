...
interface AdminRemindersProps {
  ...
  preSelectedClient?: {id: string; name: string; phone: string; email: string} | null;
}

export function AdminReminders({ navigateTo, preSelectedClient }: AdminRemindersProps) {
  ...
  useEffect(() => {
    ...
    // Then check for overdue reminders every hour
  ...
  useEffect(() => {
    if (preSelectedClient && clients.length > 0) {
      // Find the client in the loaded clients list
      const clientExists = clients.find(c => c.id === preSelectedClient.id);
      if (clientExists) {
        setSelectedClientId(preSelectedClient.id);
        setReminderMessage(`Follow up with ${preSelectedClient.name}`);
  ...
  const formatDateTimeForSubmit = () => {
    ...
    // Convert to ISO string (which automatically converts to UTC)
    return combined.toISOString();
  };

  const handleEditReminder = (reminder: Reminder) => {
    setEditingReminder(reminder);
    // Parse the UTC date and convert to local time for editing
    const date = new Date(reminder.reminderDate);
    setSelectedDate(date);
    // Get local hours and minutes (the Date object automatically converts from UTC)
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-amber-50 via-orange-50 to-rose-50">
        <LoadingSpinner message="📞 Loading your follow-up schedule..." color="amber" theme="reminders" size="lg" />
      </div>
    );
  }
